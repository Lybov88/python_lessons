...
# *** Множество (set)***
# для текста, для удаления дублирований и тд
# изменчемый тип коллекции

# Особенности
# неупорядоченный тип коллекции (объекты не индексируются)
...
# когда добавляется значение, которое уже есть во множестве
# то оно удаляется или не добавлется
my_set.add(30)

# удаление элемента (только по значению)
my_set.remove(20)
# my_set.remove(40)  # при удалении несущ значения возникает ошибка

# метод удаления без ошибок
# методы - это встроенные в объект фунцкии
# например, у собаки есть методы - 1.лаять, 2.вилять, 3.играть
...
# самостоятельно поэкспериаентировать с оставшимися методами
